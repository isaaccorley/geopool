...
\begin{figure*}[t]
  ...
  \caption{\textbf{Embedding pipeline for EuroSAT (top) and PASTIS (bottom).} In both settings, a GFM emits one embedding vector per output pixel and a pooling operator aggregates those pixel vectors into one patch or parcel representation. \emph{Top:} AEF and OlmoEarth consume a time series of image patches and return a patch of pixel-aligned embeddings; all 4{,}096 output vectors are pooled into a single patch vector for land-cover classification. \emph{Bottom:} Tessera consumes a time series for each pixel and returns a pixel embedding; only pixels inside the parcel boundary (contour shown on a PCA pseudo-RGB visualization---first three principal components of the embedding rendered as colour) are pooled into a single parcel vector for crop-type classification. The number of pixels per parcel $N$ varies from 5 to 3{,}440, making the ratio $N/D$ a key design parameter.}
  \label{fig:pipeline}
\end{figure*}
