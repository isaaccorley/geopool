...
\title{Pooling Strategies for Earth Embeddings: From Fixed Chips to Irregular Regions}
...
\IEEEPARstart{A}{} typical agricultural field in France covers 1.96\,ha---196 pixels at 10\,m resolution. A standard EuroSAT land-cover chip contains 4{,}096 pixels. Both settings aggregate pixel embeddings from a geospatial foundation model (GFM) into a single region-level vector, yet the number of pixels $N$ available for that summary differs by a factor of twenty. We show this difference, relative to the embedding dimension $D$, determines which pooling strategy works.

GFMs now publish dense pixel-level embeddings as downloadable data products~\cite{fang2026earthembeddingsproductstaxonomy,jakubik2023prithvi,cong2022satmae,bastani2023satlaspretrain,corley2025landsatbench}, enabling practitioners to reuse them without access to the underlying encoder. Many downstream tasks---crop mapping~\cite{kerner2025ftw,corley2025ftwprue,corley2025globalftw}, building detection, land-cover classification---assign a single label to a region rather than a pixel, requiring a post-hoc aggregation step (Fig.~\ref{fig:pipeline}). This \textit{input-label resolution mismatch}~\cite{workman2023handling} is ubiquitous: EuroSAT~\cite{helber2019eurosat} uses fixed $64{\times}64$ chips, while PASTIS~\cite{garnot2021panoptic} uses irregular agricultural regions. The default strategy---mean pooling---collapses each region to its average vector, discarding within-region variability that we show carries class signal.

Pooling is well-studied in image retrieval---generalized mean (GeM)~\cite{radenovic2018gem}, VLAD~\cite{jegou2010vlad}, NetVLAD~\cite{arandjelovic2016netvlad}, Fisher Vectors~\cite{perronnin2010improving}---and in temporal audio modeling~\cite{miech2017learnable}, but remains largely unexplored for GFM embeddings over fixed chips and irregular regions. The key challenge specific to this setting is that $N$ varies by three orders of magnitude across region types and sizes. When $N < D$, the sample covariance matrix has fewer data points than parameters and is mathematically rank-deficient; even at $N \approx D$ its eigenvalues are unstable, so second-order statistics estimated from such regions are unreliable. When $N \gg D$, those same statistics are well-conditioned and outperform mean pooling by a large margin. No prior work characterizes this transition or gives practitioners a decision rule.

We study post-hoc pooling strategies across the full $N/D$ spectrum. Using pixel embeddings from three GFMs on two benchmarks, we benchmark 11 training-free and 2 fitted strategies and measure how method rankings shift with region size. Our contributions:
\begin{itemize}
  \item A practitioner-facing decision rule (Table~\ref{tab:cheatsheet}, Sec.~\ref{sec:results}): the ratio $N/D$ predicts when second-order strategies (covariance, multi-moment stats) are viable, explaining the collapse of covariance pooling between fixed EuroSAT chips ($N/D{=}64$) and small irregular PASTIS regions ($N/D{\lesssim}1$).
  \item A benchmark of 11 training-free and 2 fitted strategies on EuroSAT (3 GFMs, random and spatial splits, kNN and linear probes) and PASTIS, showing distributional statistics shrink the EuroSAT random$\to$spatial accuracy gap from 9.6\,pp to 3.4\,pp on average (per-encoder reductions 43--76\%) (Sec.~\ref{sec:results}, Tables~\ref{tab:eurosat}--\ref{tab:pastis}).
...
\begin{figure*}[t]
  ...
  \caption{\textbf{Embedding pipeline for EuroSAT (top) and PASTIS (bottom).} In both settings, a GFM emits one embedding vector per output pixel and a pooling strategy aggregates those vectors into one fixed-chip or irregular-region representation. \emph{Top:} AEF and OlmoEarth consume image-patch time series and return pixel-aligned embeddings; all 4{,}096 output vectors are pooled into a chip vector for land-cover classification. \emph{Bottom:} Tessera consumes a time series for each pixel and returns a pixel embedding; only pixels inside the region boundary (contour shown on a PCA pseudo-RGB visualization---first three principal components of the embedding rendered as colour) are pooled into a region vector for crop-type classification. The number of pixels per irregular region $N$ varies from 5 to 3{,}440, making $N/D$ a key design parameter.}
  \label{fig:pipeline}
\end{figure*}
...
\textbf{Parametric (fit on train split)}: \textit{PCA} (mean-pooled then projected) and \textit{Bag of Visual Words} (\textit{BoVW})~\cite{csurka2004visual} (mini-batch $k$-means histogram), giving 11 training-free and 2 fitted strategies.
...
\input{tables/eurosat_table.tex}
\input{tables/pastis_table.tex}

\section{Results}
\label{sec:results}

\textbf{The N/D ratio governs second-order viability.} The ratio of pixels per region $N$ to embedding dimension $D$ is a proxy for the conditioning of the sample second-moment estimator: the sample covariance is mathematically rank-deficient when $N < D$ and has unstable eigenvalues until $N$ is several times $D$. This directly explains the collapse of covariance pooling between fixed EuroSAT chips and small irregular PASTIS regions; the ranking of first-order strategies (mean variants) shifts more smoothly with region size and is not predicted by $N/D$ alone. EuroSAT chips have $N{=}4{,}096$ and $D{\in}\{64,128\}$, giving $N/D \geq 32$: second-order statistics are well-conditioned. PASTIS regions have a median $N{=}196$; for OlmoEarth and Tessera ($D{=}128$) the median sits at $N/D{\approx}1.5$, and the smallest regions in the dataset fall below $N{=}D$ entirely. Fig.~\ref{fig:nd_transition} visualizes the transition; this single structural quantity explains every major finding below.
...
\textbf{When $N \approx D$: PASTIS exposes the failure case.} EuroSAT makes covariance pooling look attractive because every fixed chip supplies thousands of pixels. PASTIS tests the opposite regime: labels are irregular regions, many regions have $N < D$, and the sample covariance matrix becomes rank-deficient. In this setting, \textit{covariance} falls from 89.1\% spatial accuracy on EuroSAT-AEF to 58.7\% F1 on PASTIS-AEF, and \textit{stats} similarly loses its EuroSAT advantage (Table~\ref{tab:pastis}). The lesson is not that PASTIS produces a large new accuracy win; it shows where fixed-chip pooling advice breaks. First-order summaries such as \textit{mean+std} (linear avg.\ 56.4\%), \textit{center-weighted mean} (56.3\%), and \textit{mean} (56.1\%) are numerically close and remain competitive because they preserve coarse distributional structure without requiring stable second moments. For kNN, \textit{mean} and \textit{center-weighted mean} are the strongest PASTIS choices, so our irregular-region recommendation is a conservative first-order family rather than a single dominant strategy. This is not just an estimator choice: Ledoit-Wolf, OAS, and identity shrinkage ($\alpha{=}0.01$--$0.5$) change tiny-region linear macro-F1 by at most +0.3\,pp over sample covariance, supporting the first-order recommendation.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\linewidth]{nd_phase_transition.pdf}
  \caption{\textbf{Pooling performance as a function of $N/D$, the ratio of pixels per region to embedding dimension.} Each panel shows per-bin F1 (PASTIS, four region-size bins) and accuracy (EuroSAT) plotted against $N/D$ on a log scale for one GFM. The dashed vertical line marks $N{=}D$. To the left, where irregular regions are small and $N \approx D$, covariance (purple) collapses because its sample estimate becomes rank-deficient, while mean+std remains strong. Moving right toward fixed EuroSAT chips where $N \gg D$, covariance recovers and leads. Mean+std stays competitive across the full range.}
  \label{fig:nd_transition}
\end{figure*}

\textbf{Region-size breakdown.} Fig.~\ref{fig:ranking_heatmap} ranks methods within four irregular-region size bins averaged across all three embedding sources. For tiny regions ($N{<}50$), \textit{center-weighted mean} leads (avg.\ rank 2.0), followed by \textit{mean+max} (3.0) and \textit{stats} (4.0). For small regions ($50$--$150$\,px) \textit{center-weighted mean} again ranks first (2.0). For medium ($150$--$500$\,px) regions \textit{mean+std} ranks first (2.67); for large ($>500$\,px) regions \textit{mean+max} ranks first (1.67), with \textit{mean+std} third (3.67). \textit{Covariance} sits near the bottom for tiny and small bins (avg.\ rank ${>}11$) and recovers only to mid-pack (rank 8) in the large bin---it never leads for irregular regions, consistent with $N/D$ remaining ${\lesssim}1$--$5$ even for the largest PASTIS regions at $D{=}128$. \textit{Mean+std} is the only training-free strategy that stays in the top-3 for the small, medium, and large bins, making it the practical default for linear probes on variable-size regions; for similarity search, \textit{mean} and \textit{center-weighted mean} remain competitive defaults.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\linewidth]{ranking_heatmap.pdf}
  \caption{\textbf{Method rankings shift as irregular regions grow.} Each cell shows the average rank across AEF, OlmoEarth, and Tessera within each region-size bin---darker blue is a better rank (1=best). Center-weighted mean and mean+max dominate the tiny bin; mean+std rises as regions grow; covariance moves from near-bottom (tiny/small) to mid-pack (large) but never leads for irregular regions, since $N/D$ remains modest. Mean+std ($\bigstar$, highlighted in orange) is the only training-free method that stays in the top~3 for the small, medium, and large bins. The dashed vertical line separates per-bin results from the overall ranking.}
...
\begin{table}[!t]
  ...
  \caption{Practitioner cheat-sheet: pick a pooling strategy from $N$ (pixels per region) and $D$ (embedding dimension). Apply at inference time, no training.}\label{tab:cheatsheet}
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.36\linewidth}>{\raggedright\arraybackslash}p{0.22\linewidth}@{}}
    \toprule
...
\textbf{Recommendation.} Table~\ref{tab:cheatsheet} gives an empirical decision rule indexed by $N/D$: $N/D$ formally predicts when second-order pooling is viable, and the small/large region rows are empirical defaults supported by our two benchmarks. When $N \gg D$ (fixed chips, \eg\ $64{\times}64$ at 10\,m), use \textit{stats} or \textit{covariance} for peak accuracy; \textit{stats} is more robust to distribution shift. When $N \approx D$ or regions are variable-size (irregular polygons, fields, buildings), use \textit{mean+std}: it wins the PASTIS linear-probe average (56.4\% F1) and is the only training-free strategy that stays in the top-3 across the small, medium, and large region-size bins. For predominantly tiny regions ($N{<}D$), \textit{center-weighted mean} dominates. \textit{Mean} remains a useful lower bound. Accuracy and robustness move together here: \textit{stats} and \textit{mean+std} are simultaneously more accurate and more robust than \textit{mean}, with no observed accuracy--robustness tradeoff.
