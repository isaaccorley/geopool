...
\begin{abstract}
Geospatial foundation models (GFMs) increasingly publish pixel-level embedding products that practitioners reuse without access to the encoder. Downstream tasks with patch- or region-level labels must aggregate these dense pixel embeddings into a single vector, yet the choice of pooling operator is rarely examined. We show that one structural quantity---the ratio $N/D$ of pixels per region to embedding dimension---governs whether second-order pooling (covariance, full distributional statistics) is viable. When $N \gg D$ (e.g.\ $64{\times}64$ fixed chips), second-order statistics capture intra-region heterogeneity that mean pooling discards; when $N \approx D$ (small parcels, fields, buildings), the sample second-moment matrix is ill-conditioned and covariance collapses, while first-order distributional pooling (\emph{mean+std}, \emph{center-weighted}) remains stable. We benchmark 13 operators on EuroSAT (10 classes, $N{=}4096$) and PASTIS (18 crop classes, ${\sim}85$K parcels, median $N{=}196$) across three GFMs (AlphaEarth, OlmoEarth-Nano, Tessera). On EuroSAT, distributional statistics shrink the random$\to$spatial accuracy gap from 9.6 to 3.4\,pp averaged across encoders (per-encoder reductions 43--76\%); covariance drops from 91.0\% (EuroSAT) to 58.7\% F1 (PASTIS, AEF) while \emph{mean+std} is the only training-free method to stay in the top three across the small, medium, and large parcel-size bins. We give practitioners a one-step decision rule and release \emph{EuroSAT-Embed}, \emph{PASTIS-Embed}, and benchmark code.\footnote{Code: \url{https://github.com/isaaccorley/geopool}; data: \url{https://huggingface.co/datasets/isaaccorley/eurosat-embed} and \url{https://huggingface.co/datasets/isaaccorley/pastis-embed}.}
\end{abstract}
...
\textbf{EuroSAT-Embed.} We construct pixel-level embedding datasets from AlphaEarth~\cite{brown2025alphaearth} (AEF, 64-d int8), OlmoEarth-Nano~\cite{herzog2025olmoearth} (128-d float32, $T{=}12$ subsampled time steps), and Tessera~\cite{feng2025tesseratemporalembeddingssurface} (128-d float32). All three encoders ingest Sentinel-2 time series and emit a single annual per-pixel embedding; AEF and OlmoEarth additionally use patch spatial context. We apply the same three encoders to PASTIS parcels to produce \emph{PASTIS-Embed}. We study pooling behavior, not encoder comparison.
...
\input{eurosat_table.tex}
\input{pastis_table.tex}
...
\textbf{When $N \approx D$: second-order statistics collapse.} On PASTIS (Table~\ref{tab:pastis}), \textit{covariance} drops from 91.0\% on EuroSAT to 58.7\% F1 on AEF---a 32\,pp collapse---because many parcels have $N < D$, making the sample covariance matrix rank-deficient. \textit{Stats} similarly underperforms. In contrast, \textit{mean+std} (avg.\ 56.4\%, best on OlmoEarth at 54.9\%, second-best on AEF at 65.5\%) and \textit{center-weighted mean} (avg.\ 56.3\%) remain robust by preserving first-order distributional structure without requiring stable second moments. First-order methods (\textit{mean}, \textit{center-weighted}) are now competitive, unlike on EuroSAT. Tessera's lower absolute scores across all methods (best: 49.4\%) reflect encoder-level differences in how temporal information is distilled; the pooling-method \emph{rankings}, however, follow the same $N/D$ trend as the other two encoders.
