...
def infer_shared_pools(results_dir: Path, datasets: list[str], probe: str) -> list[str]:
    ...
    for dataset in datasets:
        csv_path = results_dir / dataset / "knn_results.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a LaTeX KNN accuracy table from results/*/knn_results.csv.",
    )
    parser.add_argument(
        "--results-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="KNN k value to select (default: 5).",
    )
    parser.add_argument(
        "--shade-min",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=(
            "KNN accuracy ($k{=}5$) across embedding sources. Cell shading indicates higher "
...
def main() -> None:
    args = parse_args()
    probe = f"knn_{args.k}"
    pools = infer_shared_pools(args.results_dir, args.datasets, probe)
    if not pools:
        raise ValueError("No shared pools found across datasets.")
    values = parse_results(args.results_dir, args.datasets, pools, probe)
